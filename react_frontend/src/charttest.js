...
class ChartTest extends Component {
     ...
     this.modeBarButtons = [[
      {
        name: 'Help',
        icon: helpIcon,
        click: () => { this.setState({question_clicked:true})  }
      },]];
   ...
   testText() {
    return (
         <div style={{width:500, height: 240, border:'solid'}}>
                         This chart blah blah
             <Button variant="contained" onClick={() => { this.setState({question_clicked:false}) }}>
                Close
             </Button>
         </div>
         )
  ...
  render() {
    return (
    <div>
      {this.state.question_clicked ? this.testText() :
      <Plot
        data={[
          {
            x: [1, 2, 3],
        ...
        layout={ {width: 600, height: 240, title: 'gfg',
